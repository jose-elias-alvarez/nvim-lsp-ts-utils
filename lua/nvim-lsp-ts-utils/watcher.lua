...
s.source = {
    get = function() return s._source end,
    set = function(val) s._source = val end,
    reset = function() s._source = nil end
}
...
local should_ignore_file = function(path)
    ...
    if vim.tbl_contains(u.tsserver_extensions, extension) then return false end

    -- the path may be a directory,
    -- but since it could be deleted, we can't check with fs_fstat
    if extension == "" then return false end
...
local should_ignore_event = function(source, target)
    ...
    local source_exists, target_exists = u.file.stat(source),
                                         u.file.stat(target)
    if source_exists then return true end
    if not target_exists then return true end
...
local handle_event_factory = function(dir)
    return function(filename)
        ...
        if should_ignore_file(path) then return end

        local source = s.source.get()
        if not source then
            s.source.set(path)
            defer(function() s.source.reset() end, 50)
            return
        end
...
M.stop = function()
    if not s.unwatch then return end
